...
#### 4. 任务取消机制
- **队列中的任务**：✅ 可直接从队列移除
- **正在执行的任务**：⚠️ 需要任务本身支持取消（通过 `shouldCancel` 标志）
- **已完成的任务**：❌ 无法取消

**实现要点**：
- Worker 中维护 `shouldCancel` 标志
- 在长时间操作的关键点检查取消标志
- 支持用户界面的取消按钮
...
class ThemeWorker extends BaseWorker {
  ...
  async handleExport(data, onProgress) {
    const steps = ['复制文件', '清理主题包', '处理资源', '压缩打包']
...
<template>
  <div>
    <el-button @click="handleExport" :disabled="isExporting">
      导出主题
    </el-button>
    <el-button v-if="isExporting" @click="handleCancel" type="danger">
      取消
    </el-button>
  </div>
</template>
...
3. **任务队列**：优先级管理 + 任务取消 + 任务重试
4. **Worker Pool**：进程池 + 负载均衡 + 崩溃恢复
5. **重量级任务迁移**：导出、导入、批处理 → 子进程
6. **任务取消机制**：支持队列取消和运行中取消
7. **完整数据流程**：从用户点击到任务完成的全链路追踪
