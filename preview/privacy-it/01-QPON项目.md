...
> 面试官问法：
> - "你在这个项目中遇到最难的技术问题是什么？"
> - "说一个你解决过的最复杂的 bug / 技术挑战"
> - "有没有让你卡住很久的问题？最后怎么解决的？"
>
> 回答要点：**不是秀方案有多复杂，而是展示你发现问题→分析根因→逐步排除→最终解决的思维过程。**

### 难点 1：低端机首屏加载 3.5 秒——常规优化全部失效后怎么办

**发现问题**：
常规优化（代码分割、懒加载、图片压缩、预加载）做完后，高端机首屏已经降到 1.5s 以内，但低端安卓机（Android 8-9、2GB 内存）仍然 3.5s。数据显示这部分用户占海外市场的 15%，流失率明显偏高。

**分析根因**：
通过分段统计发现，瓶颈不在网络，而在 **JS 解析和执行**。低端机 CPU 弱，React 框架代码 + 业务代码即使代码分割后，解析执行仍需 2-3 秒。这不是"优化 JS 体积"能解决的——低端机的硬件性能本身就是瓶颈，常规的前端优化手段已经到极限了。

**排除了哪些方案**：
- SSR：我们用 Capacitor.js 本地 WebView 加载，没有服务端，架构上不可行
- Skeleton：只是视觉安慰，用户仍然看不到真实内容，体感没有本质改善
- 预渲染：首页依赖用户位置等动态数据，静态预渲染覆盖不了

**解决思路的转折点**：
既然低端机硬件性能差是无法改变的事实，那就 **绕开性能瓶颈**——如果 **完全不跑 JS** 呢？手写一个纯 HTML/CSS 的快照页面，数据来自 localStorage 上次访问的缓存，零网络请求、零框架开销。用 iframe 嵌入做瞬开，真实页面在后台加载完后无缝替换。

**最终效果**：3.5s → 1.5s，低端机用户次日留存率提升 15%。

**为什么这是最难的**：难点不在实现（实现其实不复杂），而在 **思路的转变**——当常规优化手段在低端机硬件面前全部失效时，需要跳出"优化现有代码"的思维框架，找到"绕过硬件瓶颈"的新路径。

---

### 难点 2：iOS WKWebView 中读写 Cookie 导致视频播放中断

**发现问题**：
线上收到 iOS 用户反馈：在浏览带视频内容的页面时，视频会莫名其妙地 stop。复现后发现，只要在视频播放期间有 JS 代码读写 `document.cookie`，视频就会中断。该问题**仅 iOS 出现，Android 不受影响**。

**分析根因**：
这是 iOS WKWebView 进程架构导致的。WKWebView 的 Web 内容运行在独立进程，Cookie 存储在单独的 **Network Process** 中，JS 调用 `document.cookie` 会触发 **跨进程同步 IPC 通信**。这个同步等待会阻塞主线程几十到几百毫秒。而视频播放依赖主线程持续喂帧给渲染管线，主线程一旦被 Cookie IPC 卡住，播放就会中断。Android WebView 的 Cookie 读操作从内存缓存返回，不存在同步阻塞问题。

```
JS 调 document.cookie（读或写）
  → 同步 IPC 请求到 WKWebView Network Process
  → 主线程 hang 住等待返回（低端机可达几百 ms）
  → 视频解码器拿不到渲染帧 → 播放 stop
```

**为什么难排查**：
- 视频中断没有报错，`video.onerror` 不会触发
- `document.cookie` 是最常见的 API，埋点、登录态检查、A/B 实验等多处都在用，调用链路分散
- 在 Chrome 调试时无法复现（Chrome 的 Cookie 不走跨进程 IPC），只在真机 WKWebView 上出现

**可选方案对比**：
- **方案 A：Native 侧重写 Cookie 操作**——通过 WKWebView 的 `WKHTTPCookieStore` API 在 Native 层异步管理 Cookie，JS 侧通过 Bridge 调用读写，绕开 `document.cookie` 的同步 IPC。但需要 Native 配合改动，且所有 Cookie 操作都变成异步，业务改造成本高。
- **方案 B：移除 Cookie 依赖，迁移到 localStorage**（采用）——排查所有 `document.cookie` 调用点（埋点、登录态、A/B 实验等），统一改为 localStorage 读写。localStorage 是纯本地同步操作，不触发跨进程 IPC，从根本上消除主线程阻塞。

选 B 的原因：我们的 Cookie 使用场景全部是前端自己读写的业务数据（不涉及 HTTP 请求自动携带 Cookie 的场景），localStorage 完全可以替代，且改动范围仅在前端，不依赖 Native 配合，成本最低、最彻底。

**最终效果**：视频播放中断问题彻底解决。

**为什么值得讲**：这是一个典型的 **WebView 环境特有的坑**——纯浏览器开发时永远不会遇到，调试工具也无法复现，只能通过对 WKWebView 进程模型的理解去定位。体现了在 Hybrid App 开发中对底层运行环境的深入认知。
